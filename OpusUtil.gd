...
func get_audio_format(buffer:PackedByteArray):
	if buffer.slice(0,4) == PackedByteArray([0x4F,0x67,0x67,0x53]): return OpusTypes.AudioFormat.OGG

	if (buffer.slice(0,4) == PackedByteArray([0x52,0x49,0x46,0x46])
	and buffer.slice(8,12) == PackedByteArray([0x57,0x41,0x56,0x45])): return OpusTypes.AudioFormat.WAV
	...
	or buffer.slice(0,3) == PackedByteArray([0x49,0x44,0x33])): return OpusTypes.AudioFormat.MP3

	return OpusTypes.AudioFormat.UNKNOWN
